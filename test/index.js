...
test('Bodymen handler', t => {
  t.notOk(
    bodymen.formatter('testFormatter'),
    'should not get nonexistent formatter'
  )
  t.notOk(
    bodymen.validator('testValidator'),
  ...
  t.ok(
    schema.param('test').formatter('testFormatter'),
    'should get formatter in param'
  )
  t.ok(
    schema.param('test').validator('testValidator'),
    'should get validator in param'
  )
...
test('Bodymen middleware', t => {
  ...
  request(
    ...
    .end((err, res) => {
      ...
      t.same(
        res.body,
        { name: 'test', nestedValue: { name: 'test' } },
        'should respond with correct object'
      )
    })
